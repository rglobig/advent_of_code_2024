...
class PartOne
{
    ...
    int GetHorizontalCount(string[] lines, Func<string, int> check) => GetLinesSumRegularAndReversed(lines, check);

    int GetVerticalCount(string[] lines, Func<string, int> check)
    {
    ...
    int GetDiagonalCountRight(string[] lines, Func<string, int> check)
    {
    ...
    int GetDiagonalCountLeft(string[] lines, Func<string, int> check)
    {
        List<string> diagonals = [];
        int rows = lines.Length;
        int cols = lines[0].Length;
    ...
    int GetLinesSumRegularAndReversed(string[] lines, Func<string, int> check)
    {
        var sum = 0;
    ...
    int GetAllXmas(string line) => Regex.Matches(line, "XMAS").Count;
}
